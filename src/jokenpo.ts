 function jokenpo(userChoice) {
  ...
  function compareChoices(userChoice, cpuChoice) {
      if (userChoice === cpuChoice) return results[0];
      if (userChoice === 'Rock' && cpuChoice === 'Paper') return results[1];
      if (userChoice === 'Paper' && cpuChoice === 'Scissors') return results[1];
      if (userChoice === 'Scissors' && cpuChoice === 'Rock') return results[1];
      if (userChoice === 'Rock' && cpuChoice === 'Scissors') return results[2];
      if (userChoice === 'Scissors' && cpuChoice === 'Paper')  return results[2];
      if (userChoice === 'Paper' && cpuChoice === 'Rock')  return results[2];
    }
